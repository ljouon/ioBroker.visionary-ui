...
describe('VisionaryUiIoBrokerRepository', () => {
    ...
    it('getRooms should return IobRoomCache', async () => {
        const mockRoomsData = {
        ...
        expect(result).to.be.instanceOf(IobRoomCache);
        expect(result.values()).to.have.lengthOf(2);
    });

    it('getFunctions should return IobFunctionCache', async () => {
        ...
        expect(result).to.be.instanceOf(IobFunctionCache);
        expect(result.values()).to.have.lengthOf(2);
    });

    it('getIoBrokerStateObjects should return IobObjectCache', async () => {
        ...
        expect(result).to.be.instanceOf(IobObjectCache);
        expect(result.values()).to.have.lengthOf(2);
    });

    it('getIoBrokerStateValues should return IobStateCache', async () => {
        ...
        expect(result).to.be.instanceOf(IobStateCache);
        expect(result.values()).to.have.lengthOf(2);
    });

    it('setIobState should set state correctly', async () => {
        ...
        repository.setIobState('client1', stateId, value);
